...
class SvipPermission(BasePermission):
    def has_permission(self, request, view):
        ...
        必须是SVIP才能访问
        """
        sql = '''select user_type from authControl_userinfo where id = (select user_id from authControl_usertoken where token = %s)'''
        exsql = Exsql()
        ...
        # print('res值为：', res)
        if res != 3:
            raise exceptions.PermissionDenied('您尚未与有权限查看用户信息！！')
            return False
        return True
