...
class Authentication(BaseAuthentication):
    def authenticate(self, request):
        ...
        token_obj = authmodels.User.objects.filter(token=token).first()
        if not token_obj:
...
class SvipPermission(BasePermission):
    def has_permission(self, request, view):
        ...
        """
        try:
            sql = '''select user_type from authControlApi_userinfo where id = 
            (select user_id from authControlApi_usertoken where token = %s)'''
            exsql = Exsql()
